...
var MAX_NODES = 50
// 截图保留上限：每步截图不清理会堆积占满磁盘（ENOSPC），保留最新 N 张
var SCREENSHOT_KEEP_MAX = 10
...
function captureAndSave () {
  try {
    ...
    // 清理旧截图：每步截图若不清理会堆积占满磁盘（ENOSPC），保留最新 N 张
...
/**
 * 清理旧截图，保留最新 SCREENSHOT_KEEP_MAX 张。
 * 【为什么】perceive 每步截图存到 /sdcard/agent_screens/，不清理的话长跑堆积
 *   几百张把磁盘占满（MuMu 存储小），captureScreen 写失败 → vl_client 读失败 →
 *   agent 空转。perceive_timestamp.png 按时间戳命名，字典序=时间序，sort 后删最旧。
 */
function cleanupOldScreenshots () {
  try {
    var list = files.listDir(SCREENSHOT_DIR, function (name) { return /\.png$/.test(name) })
    if (!list || list.length <= SCREENSHOT_KEEP_MAX) return
    list.sort()
    var toRemove = list.length - SCREENSHOT_KEEP_MAX
    for (var i = 0; i < toRemove; i++) {
      files.remove(SCREENSHOT_DIR + '/' + list[i])
    }
  } catch (e) {
    console.warn('perception.cleanupOldScreenshots 清理异常：' + e)
  }
}

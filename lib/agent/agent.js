...
/**
 * 根据 task 名推断 VL 失败时该 fallback 到哪个 call_sub 子流程。
 * 【为什么】VL 不可用时 agent 不该空转 wait（架构承诺"复用 main.js 子流程"），
 *   第二次 VL 失败起绕过 VL 直接调对应子流程（main.js 已验证的规则兜底），
 *   确保 VL 断 ≠ agent 死。
 * @param {string} task
 * @returns {string|null} 子流程名（collect_own_energy/stroll），无对应则 null
 */
function deriveSubFromTask (task) {
  if (!task) return null
  if (task.indexOf('逛') >= 0 || task.indexOf('好友') >= 0) return 'stroll'
  if (task.indexOf('能量雨') >= 0) return null  // 能量雨无 call_sub（unit 定时任务）
  return 'collect_own_energy'  // 默认收自己能量
}
...
function run (task, options) {
  ...
  while (!finished && failCount < MAX_FAIL && vlFailCount < MAX_VL_FAIL) {
    ...
    // --- 1.5 VL 失败 fallback：VL 之前失败过，绕过 VL 直接调 call_sub 规则兜底 ---
    // Why: 架构承诺"VL 失败时复用 main.js 子流程"。否则 VL 断 = agent 死（wait 空转烧 token）。
    //   vlFailCount>=1 表示上次 VL 失败，这次直接用规则，不再等 VL。
    if (vlFailCount >= 1) {
      var fallbackSub = deriveSubFromTask(task)
      if (fallbackSub) {
        warnInfo('[agent] 第 ' + steps + ' 步 VL 失败 ' + vlFailCount + ' 次，fallback call_sub ' + fallbackSub + '（规则兜底）')
        var fallbackAction = { action: 'call_sub', params: { sub: fallbackSub }, reason: 'vl_failed_fallback' }
        var fallbackResult = null
        try {
          fallbackResult = executor.execute(fallbackAction)
        } catch (e) {
          errorInfo('[agent] fallback call_sub 异常：' + e)
        }
        pushHistory(history, fallbackAction, fallbackResult, perceptionResult.tree_summary)
        if (fallbackResult && fallbackResult.success) {
          logInfo('[agent] fallback call_sub 成功，任务完成')
          finished = true
          break
        }
        failCount++

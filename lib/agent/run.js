...
// agent 模式默认开启一键收：oneKeyCollectByImg 有 OCR fallback（自动定位"一键收"
// 文字按钮，不需手动框配图）。逛好友界面时会用 OCR 找按钮批量收。
// Why overwrite: 用户痛点是手动配像素框，OCR 自动定位解决。开此开关后 stroll 逛到
// 好友界面即触发 collectByOneKeyCollect（OCR 路径）。
try {

...
var SYSTEM_PROMPT = [
  ...
  '- call_sub 委派子流程精确执行（比你自己 click 坐标准）。params: {"sub": "<子流程名>"}',
  '  可用子流程：',
  '  - collect_own_energy：精确收取自己森林的能量球（内部用图像识别定位能量球中心，',
  '    凭截图猜坐标不准时务必用这个，不要自己 click 能量球）。',
  '  - stroll：逛一逛自动跳转好友森林收集能量。',
  ...
  '- "找能量"按钮：个人版主页右下橙色按钮，点击进入下一个好友。',
  '- 新版"找能量"两跳（2026活动期 07.22-09.04）：主页"找能量>"→抽奖活动页（如黄鹤楼）→活动页右下"找能量>"→好友森林。call_sub stroll 已自动两跳，无需手动处理。',
  '- "查看更多好友"：排行榜底部入口。',
  '- H5 活动：顶部有活动标题，底部有关闭/返回按钮。',
  '- 倒计时能量球：灰色/带数字，当前不可收，应 wait 或跳过。',
  '',
  '【坐标来源——最重要】点击元素时，优先用【无障碍树摘要】里给出的真实坐标',
  '（如 坐标[x1,y1][x2,y2]，点其中心 ((x1+x2)/2, (y1+y2)/2)）。',
  '只有目标元素不在树里、或标注"坐标未知"时，才凭截图估测坐标。',
  '凭图像估测坐标误差很大，能用树里的真实坐标就不要猜。',
  '【坐标系】屏幕分辨率约 1080x2400（最大 1440x2560），坐标必须在此范围内。',
  '坐标原点(0,0)在左上角。',
  '',
  '【决策原则】',
  '0. 首要按【当前任务】选主操作：',
  '   - 任务"收自己能量"：进入自己森林后 call_sub collect_own_energy，收完 finish。',
  '   - 任务"逛一逛"：call_sub stroll（自动切个人版+找能量两跳进好友森林收能量），跑完 finish。',
  '1. 弹窗/活动遮挡优先关闭返回主流程（用无障碍树给的关闭按钮真实坐标）。',
  '2. 不要自己 click 能量球坐标（Canvas 上凭截图猜必偏），一律 call_sub 子流程。',
  '3. 子流程跑完（success）就 finish，不要重复调。',
  '4. 不确定时 wait，不要乱点。'
].join('\n')
...
function GlmAnthropicExecutor (apiKey, model, endpoint) {
  ...
  this.apiKey = apiKey
  this.model = model || 'glm-5v-turbo'
  // anthropic endpoint：endpoint 参数优先（用户自定义代理/私有部署），否则默认 Coding Plan anthropic 入口
  // （createExecutor 对 glm-anthropic 不传 config.vl_endpoint，因为它默认是 OpenAI /v4 对该 key 404）
...
GlmAnthropicExecutor.prototype.inspectContent = function (responseBody) {
  try {
    var json = JSON.parse(responseBody)
...
function createExecutor () {
  ...
  if (provider === 'glm-anthropic') {
    // 不传 config.vl_endpoint（默认 /v4 会 404），让 GlmAnthropicExecutor 用默认 anthropic endpoint
    return new GlmAnthropicExecutor(apiKey, model)
  }
  // 默认通义
...
function extractDecisionJson (text) {
  ...
  // 预处理：修复 VL 常见输出瑕疵（glm-5v-turbo 省略第二个坐标 key）
  // 瑕疵: {"x": 706, 154} 缺 y → {"x": 706, "y": 154}
  //       {"dx": 0, 800} 缺 dy → {"dx": 0, "dy": 800}
  // 用 (d?x) 捕获 x 或 dx，回调里推断缺失的是 y / dy
  if (text) {
    text = text.replace(/"(d?x)"\s*:\s*(\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)\s*\}/g, function (m, key, n1, n2) {
      var otherKey = key === 'x' ? 'y' : 'dy'
      return '"' + key + '": ' + n1 + ', "' + otherKey + '": ' + n2 + '}'
    })
  }
...
function buildUserPrompt (task, perception, history) {
  ...
  if (perception && perception.tree_summary && perception.tree_summary.length > 0) {
    // 格式化每个节点为可读行，必须把 bounds 真实坐标喂给 VL
    // （之前直接 push 数组会序列化成 [object Object]，VL 拿不到坐标只能凭图瞎猜）
    lines.push('【无障碍树摘要】含元素真实坐标——点击按钮时务必用这里的坐标，不要凭图像估测')
    var nodes = perception.tree_summary
    var shown = 0
    for (var ni = 0; ni < nodes.length && shown < 30; ni++) {
      var n = nodes[ni]
      if (!n) continue
      // 跳过无文本且不可点击的节点（信息量低）
      if (!n.text && !n.clickable) continue
      var line = '- '
      if (n.text) line += '"' + n.text + '" '
      if (n.resource_id) line += 'id=' + n.resource_id + ' '
      if (n.bounds_zero) {
        line += '坐标未知 '
      } else if (n.bounds) {
        line += '坐标' + n.bounds + ' '
      }
      if (n.clickable) line += '[可点击]'
      lines.push(line)
      shown++
    }
  }
